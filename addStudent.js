...
function showServerErrors(form, errors) {
  ...
  Object.entries(errors).forEach(([fieldPath, messageOrMessages]) => {
    ...
    if (errorEl) {
      errorEl.innerHTML = messages.filter(Boolean).join("<br>");
    }
  });
}
...
function validateCurrentStepFields(form, fields) {
  ...
  fields.forEach((field) => {
    ...
    if (fieldName === "arabFullName") {
      ...
      if (!value) {
        showFieldError(field, "الاسم العربي مطلوب");
        hasErrors = true;
      } else if (names.length < 4) {
        showFieldError(field, "الاسم العربي يجب أن يكون رباعي");
        hasErrors = true;
      } else if (!names.every((name) => /^[\u0600-\u06FF]+$/.test(name))) {
        showFieldError(field, "الاسم العربي يجب أن يحتوي على حروف عربية فقط");
        hasErrors = true;
      } else {
    ...
    if (fieldName === "idNumber") {
      ...
      if (idType === "national") {
        if (!value) {
          showFieldError(field, "الرقم القومي مطلوب");
          hasErrors = true;
        ...
        if (!value) {
          showFieldError(field, "رقم جواز السفر مطلوب");
          hasErrors = true;
