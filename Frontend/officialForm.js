...
/*
 * وحدة استمارة بيانات الطالب الرسمية (منفصلة عن addStudent.js)
 * مسؤوليتها فقط: استقبال بيانات الطالب بعد الحفظ، عرضها بشكل الاستمارة
 * الرسمية المطبوعة، والتعامل مع الطباعة.
 *
 * الربط مع addStudent.js: بعد نجاح الإرسال، addStudent.js بينادي
 * OfficialForm.setPayload(payload) عشان يبعت البيانات هنا.
 */
const OfficialForm = (() => {
...
<html lang="ar" dir="rtl">
  ...
  // الواجهة العامة المتاحة لملفات JS التانية (زي addStudent.js)
  return {
    setPayload(payload) {
